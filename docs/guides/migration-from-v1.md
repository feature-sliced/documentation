...
- `/ui` — [**UI-сегмент**][disc-ui] 🔥
  - _Прежние варианты: `ui`, `components`, `view`_
- `/model` — [**БЛ-сегмент**][disc-model] 🔥
  - _Прежние варианты: `model`, `store`, `state`, `services`, `controller`_
- `/lib` — сегмент **вспомогательного кода**
  - _Прежние варианты: `lib`, `libs`, `utils`, `helpers`_
- `/api` — [**API-сегмент**][disc-api]
  - _Прежние варианты: `api`, `service`, `requests`, `queries`_
- `/config` — **сегментом конфигурации приложения** стоит
  - _Прежние варианты: `config`, `env`, `get-env`_
