...
## How to keep `entities` layer clean[​](#how-to-keep-entities-layer-clean "この見出しへの直接リンク")

### 0. Consider having no `entities` layer[​](#0-consider-having-no-entities-layer "この見出しへの直接リンク")
...
### 1. Avoid preemptive slicing[​](#1-avoid-preemptive-slicing "この見出しへの直接リンク")
...
### 2. Avoid Unnecessary Entities[​](#2-avoid-unnecessary-entities "この見出しへの直接リンク")
...
### 3. Exclude CRUD Operations from Entities[​](#3-exclude-crud-operations-from-entities "この見出しへの直接リンク")
...
### 4. Store Authentication Data in `shared`[​](#4-store-authentication-data-in-shared "この見出しへの直接リンク")
...
For more details on implementing authentication, see [the Authentication guide](/documentation/ja/docs/guides/examples/auth.md).

### 5. Minimize Cross-Imports[​](#5-minimize-cross-imports "この見出しへの直接リンク")
