...
[docs-architecture]: ../about/architecture.md
...
Очень хочется, чтобы архитектура приложения [соответствовало неким требованиям][docs-architecture], тем самым снижая воздействие типичных проблем разработки
> Но построить хорошую архитектуру без должной структуры - крайне сложно

Поэтому на данный момент в мире фронтенда нет каких-то устоявшихся подходов, которые бы встречались из проекта в проект
> Да, у нас есть принцип `Separation of Concerns`, есть `DDD`
> 
> Но даже их каждый интерпретирует *по-своему*

**Однако если отбросить детали - можно условно выделить два типа подходов к структуризации**
