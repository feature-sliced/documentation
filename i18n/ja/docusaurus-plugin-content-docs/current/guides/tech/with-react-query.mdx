...
もしエンティティ間に関係がある場合（例えば、「国」のエンティティに「都市」のエンティティ一覧フィールドがある場合）、`@x` アノテーションを使用した組織的なクロスインポートの[実験的アプローチ](https://github.com/feature-sliced/documentation/discussions/390#discussioncomment-5570073)を利用するか、以下の代替案を検討できます。
