...
Let's imagine that the `countries` slice is more dense with business logic than in the example above, then merging it with the `cities` slice will create additional clutter, so a good solution would be to restructure the logic of the `cities` slice in a higher layer.

The result of moving the logic of the `countries` and `cities` slices:
- `📂 entities`
   - `📂 countries`
	  - `📁 lib`
	    - `📄 getCountriesList `
...
By restructuring the logic of the `cities` slice on the `features` layer (not necessarily on it, any layer higher in the hierarchy will do), its domain area was more specifically clarified, and it became possible to build a correct import mechanism from the point of view of the layer hierarchy.
