...
The `entities` layer is designed as a **domain/type-centric** layer. It typically contains domain types, IDs, DTOs, and domain-level logic.
...
In practice, cross-imports in `entities` are often caused by splitting entities too granularly. Before reaching for `@x`, consider whether the boundaries should be merged instead.
