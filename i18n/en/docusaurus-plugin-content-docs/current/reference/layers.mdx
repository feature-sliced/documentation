...
Warning: not to be treated like [a utility dump](https://dev.to/sergeysova/why-utils-helpers-is-a-dump-45fo)!
...
Self-sufficient UI blocks emerged from the composition of lower-level units like entities and features.
...
If you're using a nested routing system (e.g. the router of Remix), it may be helpful to use the Widgets layer in the same way as a flat routing system would use the Pages layer — to create complete interface blocks, complete with related data fetching, loading states, and error boundaries.
