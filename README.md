...
- `ui` - набор компонентов; ui-представление модуля
- `model` - бизнес-модель; бизнес-логика модуля *(store, effects/actions, hooks/contracts, ...)*
- `lib` - вспомогательные библиотеки; инфраструктурная логика
- *(shared)* `api` - логика взаимодействия с API
