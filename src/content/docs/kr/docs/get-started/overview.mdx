...
<Aside type="caution" title="Widgets Layer 사용">

이 가이드에서는 Widgets Layer의 사용을 권장하지 않습니다. 실제 애플리케이션에서는 Widgets가 구성하는 화면 영역에 사용자 액션과 관련된 로직이 함께 포함되면서 Features와 책임이 겹치고, 두 Layer의 경계가 모호해질 수 있기 때문입니다.
