...
Astro와 FSD는 같은 `src/pages` 폴더를 다른 용도로 사용해 충돌이 발생합니다.

- **Astro**: `src/pages`는 라우트 폴더입니다. Astro가 이 폴더에서 라우트 파일을 찾습니다.
- **FSD**: `pages`는 layer 이름입니다. page 단위 코드를 이 layer에 둡니다.

이 문제를 피하려면 FSD의 pages Layer는 `📁 src/_pages`에 두고, `📁 src/pages`는 라우팅에만 사용합니다.  
이렇게 하면 Astro 라우팅은 `src/pages`에서 처리하고, FSD의 페이지 코드는 `src/_pages`에서 관리할 수 있습니다.
