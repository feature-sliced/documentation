...
단순한 라이브러리보다는 애플리케이션에 더 적합합니다. 그리고 특정 언어나 프레임워크에 제한이 없고, Monorepo 환경에서도 단계적으로 적용할 수 있습니다.
...
- shared/
  - ui/
  - api/
`📂 pages` 내부의 *home*, *article-reader*, *settings*는 **Slice**입니다. Slice는 비즈니스 도메인(이 예시에서는 각 페이지) 단위로 코드를 구분합니다. 각 Slice 안에는 ui, api, model 등의 **Segment**가 있습니다.
...
4. **Widgets** - 여러 UI 요소를 조합해 구성한 독립적인 화면 영역
...
**Widgets Layer 사용:** 이 가이드에서는 Widgets Layer의 사용을 권장하지 않습니다. 실제 애플리케이션에서는 Widgets가 구성하는 화면 영역에 사용자 액션과 관련된 로직이 함께 포함되면서 Features와 책임이 겹치고, 두 Layer의 경계가 모호해질 수 있기 때문입니다.

자세한 내용은 [Widgets Layer 가이드](/kr/docs/reference/layers#widget)를 참고하세요.

**Note:** **App/Shared** Layer는 Slice 없이 Segment로 구성됩니다.
...
2. Route를 기준으로 기존 화면을 pages Layer에 배치합니다. 화면에 종속된 UI와 로직은 우선 pages에 유지해도 됩니다.
3. Import 위반을 하나씩 해결하면서 사용자 액션은 `features`로 분리하고 비즈니스 Entity와 관련된 로직은 `entities`로 분리합니다.
