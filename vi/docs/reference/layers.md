...
**Tip:** Nếu bạn đang sử dụng nested routing system (như router của [Remix][ext-remix]), có thể hữu ích khi sử dụng layer Widgets giống như cách mà flat routing system sẽ sử dụng layer Pages — để tạo các router block đầy đủ, hoàn chỉnh với data fetching liên quan, loading state, và error boundary.
...
**Caution:** Layer này đã bị deprecated. Phiên bản hiện tại của spec khuyên tránh nó và chuyển nội dung của nó sang `features` và `app` thay vào đó.
