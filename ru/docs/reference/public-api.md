...
Кросс-импорты — это ситуация, когда один слайс импортирует из другого слайса на том же слое. Обычно это запрещено [правилом импорта для слоёв](/ru/docs/reference/layers.md#import-rule-on-layers), но часто есть реальные причины, чтобы сделать кросс-импорт. Например, в реальном мире бизнес-сущности часто ссылаются друг на друга, и лучше отразить эти отношения в коде, а не пытаться избавиться от них.
...
![Три файла, импортирующие друг друга по кругу](/ru/img/circular-import-light.svg#light-mode-only)![Три файла, импортирующие друг друга по кругу](/ru/img/circular-import-dark.svg#dark-mode-only)
